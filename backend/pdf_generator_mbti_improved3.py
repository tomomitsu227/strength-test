# backend/pdf_generator_mbti_improved.py - PDF生成修正版（エラー解消）
...
def create_radar_chart(radar_scores):
    ...
    # データ設定（修正：spider.get → radar_scores.get）
    spider.data = [[
...
def generate_pdf_report(user_id, result):
    ...
    try:
        ...
        story.append(Paragraph(f'※レーダーチャートの生成中にエラーが発生しました', body_style))
    story.append(Spacer(1, 1*cm))
    ...
    story.append(Paragraph(result.get('overall_summary', ''), summary_style))
