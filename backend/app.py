# backend/app.py - 最終完成版 (日本人平均スコア補正ロジック搭載)
...
# ★★★ 追加：日本人平均を考慮したスコア補正値 ★★★
# これは-4〜+4のスコア範囲に対する補正値です。
# 例：外向性の平均はスケール中央より少し低いので、ユーザーのスコアにプラスの補正をかけてから判定する。
BIG_FIVE_CORRECTION = {
    "Extraversion": 0.5,    # 平均が低い -> スコアを少し底上げして判定
    "Agreeableness": -0.5,   # 平均が高い -> スコアを少し下げて判定
    "Conscientiousness": 0,  # 概ね平均的
    "StressTolerance": 0.5,  # 神経症傾向が高い(ストレス耐性が低い) -> スコアを少し底上げして判定
    "Openness": 0            # 概ね平均的
}

# --- 診断ロジック (ビッグファイブの平均値を考慮) ---
    ...
    for core_rule in logic_data['main_cores']:
        ...
        if 'all' in core_rule['conditions']:
            for condition in core_rule['conditions']['all']:
                ...
                # ★★★ 変更点：ビッグファイブの特性判定時のみ、スコアを補正 ★★★
                current_score = scores[dim]
                if dim in BIG_FIVE_CORRECTION:
                    current_score += BIG_FIVE_CORRECTION[dim]

                if op == '>' and not current_score > val: is_match = False; break
                if op == '<' and not current_score < val: is_match = False; break
